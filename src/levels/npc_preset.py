...
def get_npc(dialogbox: DialogBox):
    global NPC_JOLA, NPC_BOSS, NPC_NOTE
    if NPC_JOLA is None:
        NPC_JOLA = Animation(
        ...
        NPC_BOSS = Animation(
            sheet=pygame.image.load("res/holo.png").convert_alpha(),
            cols=6,
            frame_rate=5,
        )
        NPC_NOTE = Animation(
            sheet=pygame.image.load("res/holonotatka.png").convert_alpha(),
            cols=6,
            frame_rate=5,
        )
    ...
    for i, text in enumerate(JOLA_TEXT, start=1):
        jola[(i, 0, 0)] = Tile(
            "",
            collision=False,
            interactible=NPC(dialog_box=dialogbox, animation=NPC_JOLA, text=text),
        )
    ...
    for i, text in enumerate(BOSS_TEXT, start=1):
        boss[(0, i, 0)] = Tile(
            "",
            collision=False,
            interactible=NPC(dialog_box=dialogbox, animation=NPC_BOSS, text=text),
        )
    ...
    for i, text in enumerate(NOTE_TEXT, start=1):
        note[(0, 0, i)] = Tile(
            "",
            collision=False,
            interactible=NPC(dialog_box=dialogbox, animation=NPC_NOTE, text=text),
        )
